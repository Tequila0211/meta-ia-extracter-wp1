"""
Run space extraction via Gemini API.
"""
...
def run_space_extraction(document_code: str, session=None) -> dict:
    """Run space extraction on a document.
    ...
    try:
        ...
        output_path = output_dir / "space_extraction.json"
        ...
        task_prompt = loader.get_enriched_prompt("space_extraction", document_code)
        schema_name, schema_dict = loader.get_schema_for_task("space_extraction")

        pdf_path = Path(doc.file_path)
        input_hash = compute_text_hash(document_text[:5000])

        result = client.call_with_pdf_and_images(
            task_type="space_extraction",
        ...
        # Insert spaces into DB
        if result["success"] and result["data"]:
            spaces = result["data"].get("spaces", [])
            for i, space_data in enumerate(spaces, 1):
                space_code = f"{document_code}_SP{i:02d}"
                insert_space(session, doc.id, space_code, space_data)

            update_document_status(session, doc.id, "spaces_extracted", "space_extraction")
            logger.info(f"[{document_code}] Inserted {len(spaces)} spaces")
        ...
        if manage_session:
            session.rollback()
        logger.error(f"Space extraction failed for {document_code}: {e}")
        raise
    finally:
        if manage_session:
            session.close()
