"""
Run structural mapping via Gemini API.
"""
...
def run_mapping(document_code: str, session=None) -> dict:
    """Run structural mapping on a document.
    ...
    try:
        ...
        output_path = output_dir / "mapping.json"
        ...
        task_prompt = loader.get_enriched_prompt("mapping", document_code)
        schema_name, schema_dict = loader.get_schema_for_task("mapping")

        pdf_path = Path(doc.file_path)
        input_hash = compute_text_hash(document_text[:5000])

        # Use PDF + text for mapping (overview task)
        ...
        if result["success"]:
            update_document_status(session, doc.id, "mapped", "mapping")
        ...
        if manage_session:
            session.rollback()
        logger.error(f"Mapping failed for {document_code}: {e}")
        raise
    finally:
        if manage_session:
            session.close()
