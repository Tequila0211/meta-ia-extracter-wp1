"""
Run meta-readiness audit via Gemini API.
"""
...
def run_meta_readiness(document_code: str, session=None) -> dict:
    """Run meta-readiness audit on a document.
    ...
    try:
        ...
        output_path = output_dir / "meta_readiness.json"
        ...
        task_prompt = loader.get_enriched_prompt("meta_readiness_audit", document_code)
        schema_name, schema_dict = loader.get_schema_for_task("meta_readiness_audit")

        pdf_path = Path(doc.file_path)
        input_hash = compute_text_hash(document_text[:5000])

        result = client.call_with_pdf_and_images(
            task_type="meta_readiness_audit",
        ...
        # Insert meta readiness records into DB
        if result["success"] and result["data"]:
            records = result["data"].get("readiness_records", [])
            for record_data in records:
                insert_meta_readiness(session, doc.id, record_data)

            update_document_status(session, doc.id, "meta_readiness_audited", "meta_readiness_audit")
            logger.info(f"[{document_code}] Inserted {len(records)} meta readiness records")
        ...
        if manage_session:
            session.rollback()
        logger.error(f"Meta-readiness audit failed for {document_code}: {e}")
        raise
    finally:
        if manage_session:
            session.close()
